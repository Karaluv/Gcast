...
class rendering(threading.Thread):
                ...
                if abs(y - y0/stepy) < maxR:
                    ...
                    if ch < 0.2 and zn < 0.2:
                        if wall_data[max(0, min(int(w), len(wall_data)-1))][0] > l:
                            enemy_render_data.append(
                                [l, w, enemies[i+2], enemies[i+3]])
    ...
    def ray_cast(self, map, cos, sin, cos1, sin1, minR, maxR, x0, y0):
        ...
        '''
        # block which takes variables from class
        ...
        # block which create some needed arrays
        history = []
        render_data = []
        # block which creates of horizontal and vertical rays
        mapCH = [[0 for _ in range(width)] for _ in range(height)]
        mapCV = [[0 for _ in range(width)] for _ in range(height)]
        # block which defines ray varibales
        ...
        while d < d_max:
            ...
            while l < maxR:
                ...
                if ugol == 0:
                    if l > render_data[-2][0]+1:
                        if l - 1 > minR:
                            ...
                            rX, rY, VM = history[-(1)][0], history[-(1)
                                                                   ][1], history[-(1)][2]
                ...
                if map[rY][rX] > 0:
                    ...
                    xc, yc, l = self.find_clothest_point(
                        map, l, xp, yp, xc, yc, cos/10/sc, sin/10/sc, dl/10/sc, dl/1000/sc)
                    rY, rX = int(yc), int(xc)
                    dx, dy = abs(rX - xc), abs(rY - yc)

                    if yp != rY and xp != rX:
                        k = max(dy, dx)
                        xp, yp, l = self.find_clothest_gran(
                            l, xp, yp, xc, yc, cos/10/sc, sin/10/sc, dl/10/sc, dl/1000/sc)
                        yp, xp = int(yp), int(xp)
                    ...
                    if not VM:
                        mapCH[rY][rX] += 1
                        render_data[-1] = [l, d*dW,
                                           mapCH[rY][rX], k, map[rY][rX]]
                    else:
                        mapCV[rY][rX] += 1
                        render_data[-1] = [l, d*dW,
                                           mapCV[rY][rX], k, map[rY][rX]]
    ...
    def draw_wall(self, render_data, enemies, elevation):
        ...
        for i in range(i_max):
            if j < len(enemies):
                # draws enemy if he is standing in front of the current wall
                if render_data[i][0] < enemies[j][0]:

                    render_image = pygame.transform.scale(sprites[enemies[j][2]][int(
                        enemies[j][3])], (int(pe/enemies[j][0]), int(pe/enemies[j][0]*1.4)))

                    render_mask = pygame.mask.from_surface(render_image)
                    render_mask = render_mask.to_surface()
                    dark = pygame.Surface(
                        render_image.get_size()).convert_alpha()
                    dark.fill((0, 0, 0, min(255, enemies[j][0]*255/10)))
                    render_image.blit(dark, (0, 0))
                    render_image.blit(render_image, (0, 0),
                                      None, pygame.BLEND_RGBA_MULT)
                    inv = pygame.Surface(
                        render_mask.get_rect().size, pygame.SRCALPHA)
                    ...
                    self.render_surface.blit(inv, (int(
                        enemies[j][1]*dW-pe/enemies[j][0]/2), int(H//2*elevation-pe/enemies[j][0]*0.4)))
                    self.render_surface.blit(render_image, (int(
                        enemies[j][1]*dW-pe/enemies[j][0]/2), int(H//2*elevation-pe/enemies[j][0]*0.4)))
                    j += 1
            # draws column of the wall it self
            self.render_surface.blit(render_data[i][5], (render_data[i][1], int(
                H//2*elevation-pe/render_data[i][0])))
    ...
    def draw_background(self, elevation, maxR):
        ...
        self.ground = pygame.Surface(
            (W, H//2-H//2*(elevation-1)-self.pe/maxR+1))
        # creates gradient rectangle
        self.gradientRect(self.sky, (153, 153, 102), (0, 0, 0), pygame.Rect(
            0, 0, W, H//2+H//2*(elevation-1)-self.pe/maxR+1))
        self.gradientRect(self.ground, (0, 0, 0), (135, 62, 35), pygame.Rect(
            0, 0, W, H//2-H//2*(elevation-1)-self.pe/maxR+1))
        ...
        self.render_surface.blit(
            self.ground, (0, H//2+H//2*(elevation-1)+self.pe/maxR-1))
    ...
    def render(self, args):
        ...
        render_wall_data = self.ray_cast(
            map, cos0, sin0, cos1, sin1, minR, maxR, x0, y0)
        # creates enemy data
        enemy_render_data = self.enemy_ray_caster(
            render_wall_data, enemies, cos0, sin0, cos1, sin1, minR, maxR, x0, y0)
        ...
        # waits while screen will be availble
        while self.blit:
            pass
        # draws rendred stuff
        self.blit = True
    ...
    def texturize(self, render_data):
        ...
        for j in range(len(n_)):
            ...
            texture_buffer.blit(
                ston[render_data[end][4]-1][0], (0, 0), (k1*width_, 0, width_, height_))
            dark.fill((0, 0, 0, render_data[x][0]*255/7))

            for i in range(n_[j]):

                h = int(2*pe/render_data[x][0])
                # cheks if texture is bigger than screen
                if h > H:
                    texture_data = pygame.Surface((dW, H))
                    texture_buffer_ = texture_buffer.subsurface(
                        (0, ((h-H)//2/h)*height, width, H/h*height))
                    texture_data.blit(pygame.transform.scale(texture_buffer_, (round(
                        dW*n_[j]), round(H))), (0, 0), (round(dW*i), 0, round(dW), round(H)))
                    ...
                    texture_data = pygame.Surface((dW, h))
                    texture_data.blit(pygame.transform.scale(
                        texture_buffer, (round(dW*n_[j]), h)), (0, 0), (round(dW*i), 0, dW, h))
