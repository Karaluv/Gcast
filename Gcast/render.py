...
class rendering:
    ...
    def __init__(self, u, density, dl, render_surface, height, width):
        ...
        self.gradientRect(self.sky, (60, 60, 60), (0, 0, 0), pygame.Rect(0, 0, W, H // 2 - maxR / self.pe))
        self.gradientRect(self.ground, (0, 0, 0), (115, 115, 115), pygame.Rect(0, 0, W, H // 2 - maxR / self.pe))

        self.width = width
        self.height = height

    @lru_cache()
    def find_closest_point(self, map, l, xp, yp, xc, yc, cos, sin, lc, lmin):
        ...
        if lc > lmin:
            return self.find_closest_point(map, l + lc, xp, yp, xc + cos, yc + sin, cos / 10, sin / 10, lc / 10, lmin)

    @lru_cache()
    def find_closest_gran(self, l, xp, yp, xc, yc, cos, sin, lc, lmin):
        ...
        if lc > lmin:
            return self.find_closest_gran(l + lc, xp, yp, xc + cos, yc + sin, cos / 10, sin / 10, lc / 10, lmin)
    ...
    def ray_cast(self, map, a0, a1, minR, maxR, x0, y0):
        ...
        # тут создаешь как хочешь tuple размером (int(1/density)+1) строк и в каждой строке 6 элементов: все из них
        # по умолчанию 0 в этой функции ты лишь записываешь и меняешь первые 5 из них l, t*w-d,mapCV[rY][rX],k,
        # map[rY][rX]
        ...
        while t < 1:
            ...
            while l < maxR:
                ...
                if map[rY][rX] > 0:
                    ...
                    xc, yc, l = self.find_closest_point(map, l, xp, yp, xc, yc, cos / 10 / sc, sin / 10 / sc,
                                                        dl / 10 / sc, dl / 100 / sc)
                    rY, rX = int(yc), int(xc)
                    dx = abs(rX - xc)
                    dy = abs(rY - yc)
                    ...
                    if yp != rY and xp != rX:
                        k = max(dy, dx)
                        xp, yp, l = self.find_closest_gran(l, xp, yp, xc, yc, cos / 10 / sc, sin / 10 / sc,
                                                           dl / 10 / sc, dl / 100 / sc)
                        yp = int(yp)
                        xp = int(xp)
    ...
    def draw_wall(self, a0, a1, render_data, enemies):
        ...
        if i_max > len(render_data) - 1:
            i_max = len(render_data) - 1
        while i <= i_max:
            if j < len(enemies):
                if render_data[i][0] < enemies[j][0]:
                    render_image = pygame.transform.scale(sprites[enemies[j][2]][int(enemies[j][3])],
                                                          (int(pe / enemies[j][0]), int(pe / enemies[j][0] * 1.4)))
                    ...
                    self.render_surface.blit(render_image, (
                        int(enemies[j][1] * (dw - 1) - pe / enemies[j][0] / 2), int(H // 2 - pe / enemies[j][0] * 0.4)))
                    j += 1
        ...
        for j in range(len(n_)):
            ...
            for i in range(n_[j]):
                ...
                if h > H:
                    ...
                    texture_buffer_ = texture_buffer.subsurface((0, ((h - H) // 2 / h) * texture_buffer.get_height(),
                                                                 texture_buffer.get_width(),
                                                                 H / h * texture_buffer.get_height()))
                    texture_data.blit(pygame.transform.scale(texture_buffer_, (round(dw * n_[j]), H)), (0, 0),
                                      (round(dw * i), 0, dw, H))
                    ...
                    texture_data.blit(pygame.transform.scale(texture_buffer, (round(dw * n_[j]), h)), (0, 0),
                                      (round(dw * i), 0, dw, h))
